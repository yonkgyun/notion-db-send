...
- 새로고침 옆 톱니바퀴에서 네 칸의 이름과 연결 링크를 각각 설정할 수 있습니다. 상단 두 칸의 집계 대상은 기존 할일/노트 데이터베이스로 유지되며, 링크만 별도로 변경됩니다. 아래 두 칸은 링크를 넣으면 바로가기가 됩니다.
- 설정은 해당 기기/브라우저의 사이트 저장 공간에 저장됩니다. 같은 주소의 탭끼리는 반영되지만 다른 기기/브라우저로 자동 동기화되지는 않으며, 사이트 데이터를 지우면 초기화됩니다. 링크를 비우면 해당 칸의 이동 기능을 끌 수 있고, `기본값`을 누른 뒤 `설정 저장`으로 원래 링크를 복원합니다.
- 카드 제목과 아래 입력 영역은 화면상 13px로 통일했습니다. iPhone 자동 확대를 방지하기 위해 실제 입력 요소는 16px을 유지하고 시각적으로 축소합니다. 제목/메모는 두 줄 높이이며 본문은 내용에 맞게 늘어납니다.
