...
- 상단 2x2 대시보드는 기본적으로 오늘 기록한 할일/노트 수와 아래 빈 두 칸으로 시작합니다. 네 칸 모두 각각 이름, 링크, 집계 대상과 기준을 설정할 수 있습니다.
- 기본 기준인 '오늘 등록한 수'는 한국 시간(Asia/Seoul) 오늘 00:00부터 다음 날 00:00 전까지 **새로 생성된 페이지**를 집계합니다. 할일의 지정 날짜/완료 여부와 무관하며, 이 앱 외에 Notion에서 직접 기록한 항목도 포함합니다. 휴지통 항목은 제외합니다.
...
- 새로고침 옆 톱니바퀴에서 네 칸의 이름과 연결 링크, 표시할 숫자, 집계 데이터베이스를 각각 설정할 수 있습니다. 이동 링크와 집계 대상은 서로 독립적입니다.
- 집계 기준은 오늘 등록한 수, 전체 기록 수, 날짜가 오늘인 수, 미완료/완료 체크박스 수, 특정 선택·상태 값의 수, 숫자 숨김입니다. 조건은 카드별 한 가지이며, Notion 보기 URL의 필터는 자동 적용되지 않습니다.
- 기존 할일/노트 또는 다른 데이터베이스 원본 링크/ID를 집계 대상으로 지정할 수 있습니다. 다른 데이터베이스도 기존 Notion 연결에 공유해야 하며, 일반 페이지/대시보드 링크로는 집계할 수 없습니다. 새 API 키는 필요하지 않습니다.
- 설정 상단의 `집계 기준 가이드`를 펼치면 기준별 의미, 속성 이름과 유형의 차이, 예시, 오류 확인 방법을 볼 수 있습니다. 기존 이름·링크 설정은 업데이트 후에도 유지됩니다.
